...
export class TopCutNFT extends ethereum.SmartContract {
  static bind(address: Address): TopCutNFT {
    return new TopCutNFT("TopCutNFT", address);
  }

  TOPCUT_VAULT(): Address {
    let result = super.call("TOPCUT_VAULT", "TOPCUT_VAULT():(address)", []);

    return result[0].toAddress();
  }

  try_TOPCUT_VAULT(): ethereum.CallResult<Address> {
    let result = super.tryCall("TOPCUT_VAULT", "TOPCUT_VAULT():(address)", []);
    if (result.reverted) {
  ...
  mintPriceETH(): BigInt {
    let result = super.call("mintPriceETH", "mintPriceETH():(uint256)", []);

    return result[0].toBigInt();
  }

  try_mintPriceETH(): ethereum.CallResult<BigInt> {
    let result = super.tryCall("mintPriceETH", "mintPriceETH():(uint256)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
